...
class MAE3D(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=1, num_frames=96, temp_stride=2,
...
def mae3d_vit_base_patch16_stride16_dec512d4b(**kwargs):
    model = MAE3D(
        img_size=160,temp_stride=16,patch_size=16, embed_dim=768, depth=12, num_heads=12,
        decoder_embed_dim=512, decoder_depth=4, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model 

def mae3d_vit_large_patch16_stride16_dec512d4b(**kwargs):
    model = MAE3D(
        img_size=160,temp_stride=16,patch_size=16, embed_dim=1024, depth=24, num_heads=16,
        decoder_embed_dim=512, decoder_depth=4, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model 

# set recommended archs
mae3d_base_16 = mae3d_vit_base_patch16_stride16_dec512d4b 
mae3d_large_16 = mae3d_vit_large_patch16_stride16_dec512d4b
