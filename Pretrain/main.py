...
from builtins import enumerate, float, int, list, print, str
import argparse
import os
...
from utils.mae_utils import get_loader
...
def main():
    ...
    def train(args, global_step, train_loader, val_best, scaler):
        ...
        for step, batch in enumerate(train_loader):
            ...
            if args.distributed:
                if dist.get_rank() == 0:
                    print("Step:{}/{}, Loss:{}, Time:{}".format(global_step, args.num_steps, loss, time() - t1))
            else:
                print("Step:{}/{}, Loss:{}, Time:{}".format(global_step, args.num_steps, loss, time() - t1))
            ...
            if val_cond:
                ...
                if val_loss_recon < val_best:
                    ...
                    print(
                        "Model was saved ! Best Recon. Val Loss: {}, Recon. Val Loss: {}".format(
                            val_best, val_loss_recon
                        )
                    )
                else:
                    print(
                        "Model was not saved ! Best Recon. Val Loss: {} Recon. Val Loss: {}".format(
                            val_best, val_loss_recon
                        )
                    )
    ...
    def validation(args, test_loader):
        ...
        with torch.no_grad():
            for step, batch in enumerate(test_loader):
                ...
                print("Validation step:{}, Loss:{}, Loss Reconstruction:{}".format(step, loss, loss_recon))
    ...
    train_loader, test_loader = get_loader(args)[0] , get_loader(args)[1]
...
if __name__ == "__main__":
    main()
# python -m torch.distributed.launch --nproc_per_node=3 --master_port=11223 main.py --batch_size=1 --num_steps=100000 --lrdecay --eval_num=500 --lr=6e-6 --decay=0.1